...
def acomodar(s: list[str]) -> list[str]:
    ...
    lista_completa = lista_up + lista_lla
...
def pos_umbral(s: list[int], u: int) -> int:
    cont: int = 0
    sum_umbral: int = 0
    umbral_superado: bool = False

    for i in s:
        if i >= 0:
            sum_umbral += i

        if sum_umbral >= u and not umbral_superado:
            umbral_superado = True
        
        if not umbral_superado:
            cont += 1
...
def suma_puestos(ls: list[list[int]], naciones: list[str]):
...
def cuenta_posiciones_por_nacion (naciones: list[str], torneos: dict[int, list[str]]):
    pos_naciones: dict = {}
    listas_pos: list = []
    pos_pais: list = []
    
    for n in naciones:
        for a in torneos.values():
            for c in a:
                if n == c:
                    pos_pais.append(1)
                else:
            ...
            listas_pos.append(pos_pais)
            pos_pais = []
                
        pos_naciones[n] = suma_puestos(listas_pos,naciones)
        listas_pos = []
                

    return pos_naciones

print(suma_puestos([[1,2,3],[1,2,3]], ["arg", "aus", "nz"]))
